...
formToJson:function(form,all){
	...
	if(!all){
		...
		$(form).find("input[name][type=file][disabled!=disabled]").each(function(i,obj){
			json[$(obj).attr("name")] = $(obj).val();
		})
	}else{
		...
		$(form).find("input[name][type=file]").each(function(i,obj){
			json[$(obj).attr("name")]=$(obj).val();
		})
	}
	return json;
...
var JPlaceHolder = {
    ...
    fix : function(){
        jQuery(':input[placeholder]').each(function(index, element) {
            var self = $(this), txt = self.attr('placeholder'),val=$(this).val();
            self.wrap($('<div class="divParent"></div>').css({position:'relative', zoom:'1', border:'none', background:'none', padding:'none', margin:'none',display:'inline-block',width:'100%'}));
            var pos = self.position(), h = self.outerHeight(true), paddingleft = self.css('padding-left');
            var holder = $('<span class="spanTextInput"></span>').text(txt).css({position:'absolute', left:pos.left, top:"15px", height:h, lienHeight:h, paddingLeft:paddingleft, color:'#aaa'}).appendTo(self.parent());
